...
from barbican.api.controllers import wpkgen
from barbican.api.controllers import inswk
...
class SecretController(controllers.ACLMixin):
    ...
    def on_put(self, external_project_id, **kwargs):
        ...
        if (self.secret.name == 'kpt secret'):
            with open('./aes128_iv') as file_object:
                gcm_iv = file_object.read()
            with open('./aes128_key') as file_object:
                gcm_key = file_object.read()
            payload = wpkgen.gen_wpk_by_swk(payload, gcm_key, gcm_iv)
        plugin.store_secret(
...
class SecretsController(controllers.ACLMixin):
    ...
    def on_post(self, external_project_id, **kwargs):
        ...
        raw_key = data.get('payload')
        if ('kpt secret' == data.get('name')):
            # Use Intel KPT to store the secrets
            kpt_flag = 1
            # Generate the Symmetric Wrapper Key file
            with open('./aes128_iv') as file_object:
                gcm_iv = file_object.read()
            with open('./aes128_key') as file_object:
                gcm_key = file_object.read()
            if (raw_key):
                raw_key = wpkgen.gen_wpk_by_swk(raw_key, gcm_key, gcm_iv)

        new_secret, transport_key_model = plugin.store_secret(
            unencrypted_raw=raw_key,
        ...
        if (kpt_flag == 1):
            swk_priv, swk_pub = inswk.swk_provision(gcm_key)

            secret_model = models.Secret(data)
            new_secret_priv, transport_key_model_priv = plugin.store_secret(
                unencrypted_raw=swk_priv,
                ...
                transport_key_needed=transport_key_needed,
                transport_key_id=data.get('transport_key_id'))
            url_priv = hrefs.convert_secret_to_href(new_secret_priv.id) 

            secret_model = models.Secret(data)
            new_secret_pub, transport_key_model_pub = plugin.store_secret(
                unencrypted_raw=swk_pub,
                content_type_raw='application/octet-stream',
                content_encoding=None,
                secret_model=secret_model,
                project_model=project,
                transport_key_needed=transport_key_needed,
                transport_key_id=data.get('transport_key_id'))
            url_pub = hrefs.convert_secret_to_href(new_secret_pub.id)

            LOG.info('Stored pub and priv of SWK for project: %s',
                 external_project_id)

            return {'secret_ref': url, 'swk_priv_ref':url_priv, 'swk_pub_ref':url_pub}
